...
def download_files(first_url,output_dir):
    ...
    first_index = re.findall(r'[0-9]+', url_tail)[-1]#ind last groupe of digit
    index_count , error_count = 0,0
    while (error_count<5):
        next_index = str(int(first_index)+index_count)
        if first_index[0] == '0' : # ero pading
        ...
        try :
            ...
            print( output_file +"\t"+next_url)
            urllib.request.urlretrieve(next_url,output_file)
            print("ok")
            print("Seccessfully donloaded{}",format(os.path.basename(next_url)))
        except IOError :
...
download_files('http://699340.youcanlearnit.net/image001.jpg','.\\hi')
